...
current_dir = os.path.dirname(os.path.abspath(__file__))  # 获取 baseline 目录
parent_dir = os.path.dirname(current_dir)  # 获取 schema_guided_dst 目录
root_dir = os.path.dirname(parent_dir)  # 获取项目根目录
sys.path.append(root_dir)  # 将项目根目录添加到路径
...
class TqdmHook(session_run_hook.SessionRunHook):
    def __init__(self, max_steps, update_steps=100):
        """
        Args:
            max_steps: 最大训练步骤数。
            update_steps: 每多少步更新一次进度条。
        """
    ...
    def before_run(self, run_context):
        # 请求 `global_step` 的值
        return session_run_hook.SessionRunArgs(fetches=tf.train.get_global_step())

    def after_run(self, run_context, run_values):
        current_step = run_values.results
        if self.pbar:
            if current_step > self.pbar.n:
                self.pbar.update(current_step - self.pbar.n)
...
def _model_fn_builder(bert_config, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if mode == tf_estimator.ModeKeys.TRAIN:
            ...
            # 定义 TqdmHook
            tqdm_hook = TqdmHook(max_steps=num_train_steps, update_steps=100)
