        ...
        for slot_span in char_slot_spans:
            if slot_span["slot"] == slot:
                try:
                    ...
                    # Check if start position exists in alignments
                    if slot_span["start"] not in alignments:
                        print(f"Warning: start position {slot_span['start']} not found in alignments for slot {slot}")
                        continue
                        
                    # Check if end position exists in alignments
                    end_pos = slot_span["exclusive_end"] - 1
                    if end_pos not in alignments:
                        # Try to find the closest valid end position
                        while end_pos >= slot_span["start"]:
                            if end_pos in alignments:
                                break
                            end_pos -= 1
                        else:
                            print(f"Warning: No valid end position found for slot {slot}")
                            continue
                    
                    start_tok_idx = alignments[slot_span["start"]]
                    end_tok_idx = alignments[end_pos]
                    
                    # Validate token indices
                    if start_tok_idx < 0:
                        print(f"Warning: Invalid start token index {start_tok_idx} for slot {slot}")
                        continue
                        
                    if start_tok_idx >= len(subwords):
                        print(f"Warning: Start token index {start_tok_idx} exceeds subwords length {len(subwords)} for slot {slot}")
                        continue
                        
                    # Ensure end_tok_idx is within bounds
                    ...
                    if start_tok_idx <= end_tok_idx:
                        ...
                        print(f"Warning: Invalid span (start > end) for slot {slot}")
